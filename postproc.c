...
//alpha (34)
//rad. viscosity energy flux (35) -> average rho
//rho-weighted minus radial velocity in the outflow (36) -> rho-weighted eccentricity for problem 87, 145
...
int calc_radialprofiles(ldouble profiles[][NX])
{
  ...
  ldouble normalize[NX], rho134[NX], pgas134[NX], bfield134[NX], uconphi134[NX], ptot134[NX], betainv134[NX],scaleheight134[NX],rholambda134[NX],Qthetaweight[NX];
  ldouble thetamin_134=M_PI/3.;
  ldouble thetamax_134=2*M_PI/3.;
  ldouble sigmagdet[NX];
  
  //special quantities for Reynolds stress
  ldouble rhodisk[NX],urdisk[NX],uthdisk[NX],uphidisk[NX]; //mean density, ur, uphi
  
  //extra quantities for mean theta H/R
  ldouble thm[NX][NZ];  
  ...
  ldouble rhoucont,enden,rhouconr,rhouconrucovphi,Tij[4][4],Tij22[4][4],Rij[4][4],Rviscij[4][4],Trt,Trphi,Fluxx[NV],Rrt,Rtt,Rrphi,Ttt,Rviscrt,bsq,bcon[4],bcov[4];
  ldouble Trtmagn,Trtkin,Trtuint,endensimple,endenuconr,endenuconth,Omega,TrphiRey,TrphiMax;
  ...
  ldouble du[NV],dul[NV],dur[NV],aaa[24],ahd,arad,Jvec[3],Jvec_jet[3],xjet_top[3],xjet_bot[3],jetpos_weight;
  ldouble Gi[4],Giff[4];
  ldouble ppMink[NV],TijMink[4][4],RijMink[4][4],Ssig[4],Ssigcov[4]; //Quantities for angular momentum
  ldouble rbary,rbaryweight,mass;
  ldouble Rtilt_num,Rtilt_den,Rprec_num,Rprec_den; //Radiation flux tilt and precetion from Asahina and Ohsuga 2024
...
//#pragma omp parallel for private(ix)
  for(ix=0;ix<NX;ix++)
  {
    ...
    Jvec_jet[0]=Jvec_jet[1]=Jvec_jet[2]=0.;
    xjet_top[0]=xjet_top[1]=xjet_top[2]=0.;
    xjet_bot[0]=xjet_bot[1]=xjet_bot[2]=0.;
    Ssig[0]=Ssig[1]=Ssig[2]=Ssig[3]=0.;
    rbary=rbaryweight=jetpos_weight=0.;
    Rtilt_num=Rtilt_den=Rprec_num=Rprec_den=0.;
    ...
    for(iz=0;iz<NZ;iz++)
    {
      mass=0.; //initialize weight
      thm[ix][iz]=0.; //initialize thm array
    
      //for(iy=0;iy<NY;iy++)
      for(iy=NCCORRECTPOLAR+2;iy<NY-(NCCORRECTPOLAR+2);iy++)
      {
        //metric
        pick_g(ix,iy,iz,gg);
        pick_G(ix,iy,iz,GG);
	...
	//transforming interpolated primitives to BL
        //trans_pall_coco(pp,ppMink,MYCOORDS,MINKCOORDS,xx,&geom,&geomMink); //for angmom
	//trans_pall_coco(pp,ppMink,MYCOORDS,OUTCOORDS,xx,&geom,&geomBL);
        #ifdef PRECOMPUTE_MY2OUT
	...
	//trans_pall_coco_my2out(fd_pm1,fd_pm1,&geomm1,&geomBLm1); // don't need these anymore
        ...
        if(doingavg)
        {
          ...
          TrphiRey = (rho + GAMMA*uint)*utcon[1]*utcon[3];
          TrphiMax = bsq*utcon[1]*utcon[3] - bcon[1]*bcon[3];       
	...
	betagamma2=(-Trt/rhouconr)*(-Trt/rhouconr) - 1.;
        ...
        isjet=0;
        isdisk=0;
        if(0.5*bsq/rho > 1. || Be >= 0.05)
        {
          isjet=1;
        }
	if(0.5*bsq/rho <= 1. || Be <= 0.05) isdisk=1;
        //if(0.5*bsq/rho <= 1. && Be <0) isdisk=1;
	...
	//}
        ...
        ldouble alpha = sqrt(geomBL.gg[1][1]*geomBL.gg[3][3])*bcon[1]*bcon[3]/ptot;
        ldouble alphaRey = TrphiRey/ptot;
        ldouble alphaMax = alpha;//;TrphiMax/ptot;
        ...
        if(isdisk==1)
        {
          rhodisk[ix]+=rho*dx[1]*dx[2]*geomBL.gdet;
          urdisk[ix]+=rho*utcon[1]*dx[1]*dx[2]*geomBL.gdet;
          uthdisk[ix]+=rho*utcon[2]*dx[1]*dx[2]*geomBL.gdet;
          uphidisk[ix]+=rho*utcon[3]*dx[1]*dx[2]*geomBL.gdet;
        
          profiles[32][ix]+=alphaMax*rho*dxph[1];
          profiles[33][ix]+=alphaRey*rho*dxph[1]; //swicthed with rad viscosity energy flux (35)
        }
        //profiles[33][ix]+=rho/NY; //swicthed with rad viscosity energy flux (35)
	...
	//rho-weighted beta (32)
...
#else
        profiles[31][ix]+=rho*sqrt(bsq)*dxph[1];
#endif
        ...
        //if(Be<0. && utcon[1]<0.) profiles[29][ix]+=dx[1];//rho*dxph[0]*dxph[1]*dxph[2]*dx[1];
        //mass+=rho*dxph[0]*dxph[1]*dxph[2];
        profiles[29][ix]+=rho*dxph[1]*pow(fabs(M_PI/2.-xxBL[2]),2.);
#endif
        ...
        //sqrt(b^2*rho) weighted q-theta (28)
        //profiles[26][ix]+=sqrt(bsq*rho)*qtheta*dxph[1];
        //Qthetaweight[ix]+=sqrt(bsq*rho)*dxph[1];

        if(isdisk==1)
        {
          profiles[26][ix]+=qtheta*dxph[1];
          Qthetaweight[ix]+=dxph[1];
          profiles[45][ix]+=qphi*dxph[1];
        }
        
        //if(ix==50) printf("%i %i %f %f %f \n",ix,iy,qtheta,profiles[26][ix],Qthetaweight[ix]);
	...
	rholambda134[ix] += rho*lambdaMRI* dx[1]*dx[2]*geomBL.gdet; 
        ...
        if(bsq/rho < 1.) profiles[55][ix]+=(Trphi+Rrphi)*dx[1]*dx[2]*geomBL.gdet; 
        //else profiles[55][ix]+=(Trphi)*dx[1]*dx[2]*geomBL.gdet;
        
        //integrated Trphi (58)
        //profiles[56][ix]+=(Trphi-rhouconrucovphi)*Omega*dx[1]*dx[2]*geomBL.gdet;
        profiles[56][ix]+=(Trphi+Rrphi)*dx[1]*dx[2]*geomBL.gdet;        
        ...
        //jet internal energy flux (48)
        if(isjet==1)
          profiles[46][ix]+=(-Trtuint)*dx[1]*dx[2]*geomBL.gdet;
        
        //kinetic  mhd energy flux (49)
        //profiles[47][ix]+=(-Trtkin)*dx[1]*dx[2]*geomBL.gdet;
        
        //kinetic + binding mhd energy flux in jet (49)
        if(isjet==1)
          profiles[47][ix]+=(-Trtkin - rhouconr)*dx[1]*dx[2]*geomBL.gdet;
	...
	profiles[15][ix]+=(-Rrt)*dx[1]*dx[2]*geomBL.gdet;
        ...
        #if(PROBLEM==87 || PROBLEM==93 || PROBLEM==145)
        ...
        //calc_Tij(ppMink,&geomMink,TijMink); //T^munu in Minkowski coords
        //#ifdef RADIATION
        //calc_Rij(ppMink,&geomMink,RijMink); //R^munu in Minkowski coords
        //#endif
        
        //if(ix==20) printf("ix iy rhoM uintM ux uy uz rho uint ur uth uphi - %i %i %e %e %e %e %e %e %e %e %e %e\n",ix,iy,ppMink[0],ppMink[1],ppMink[VX],ppMink[VY],ppMink[VZ],pp[0],pp[1],pp[VX],pp[VY],pp[VZ]);
        
        ldouble Jx,Jy,Jz,th,ph,r;
        ldouble xxveccar[4],velvec[3],posvec[3],jvec[3],jvecR[3];
        ldouble posvecT[3],velvecT[3],jvecT[3],thp,sgas_inside,sgas,jet_weight;
        
        //Need to construct ucon,bcon,Fcon by hand... JC->MINK broken?
        ldouble uconM[4],bconM[4],FluxM[3],FluxBL[3],Rijtemp[4][4];
        
        //Vel. in Mink
        uconM[0]=utcon[0];
        uconM[1]=sin(th)*cos(ph)*utcon[1]
        + cos(th)*cos(ph)*utcon[2]*r
        - sin(ph)*utcon[3]*r*sin(th);
        uconM[2] = sin(th)*sin(ph)*utcon[1]
        + cos(th)*sin(ph)*utcon[2]*r
        + cos(ph)*utcon[3]*r*sin(th);
        uconM[3] = cos(th)*utcon[1]
        - sin(th)*utcon[2]*r;
     
        //Mag field in Mink
        bconM[0]=bcon[0];
        bconM[1]=sin(th)*cos(ph)*bcon[1]
        + cos(th)*cos(ph)*bcon[2]*r
        - sin(ph)*bcon[3]*r*sin(th);
        bconM[2] = sin(th)*sin(ph)*bcon[1]
        + cos(th)*sin(ph)*bcon[2]*r
        + cos(ph)*bcon[3]*r*sin(th);
        bconM[3] = cos(th)*bcon[1]
        - sin(th)*bcon[2]*r;
        
        //Rad field in Mink
        calc_Rij(pp,&geomBL,Rijtemp);
        FluxBL[0]=Rijtemp[1][0];  
        FluxBL[1]=Rijtemp[2][0];  
        FluxBL[2]=Rijtemp[3][0]; 
        
        //Rad field in Mink
        FluxM[0]=sin(th)*cos(ph)*FluxBL[0]
        + cos(th)*cos(ph)*FluxBL[1]*r
        - sin(ph)*FluxBL[2]*r*sin(th);
        FluxM[1] = sin(th)*sin(ph)*FluxBL[0]
        + cos(th)*sin(ph)*FluxBL[1]*r
        + cos(ph)*FluxBL[2]*r*sin(th);
        FluxM[2] = cos(th)*FluxBL[0]
        - sin(th)*FluxBL[1]*r;
        
        //if(ix==20) printf("ix iy ux uy uz ur uth uphi R01 R10 - %i %i  %e %e %e %e %e %e %e %e\n",ix,iy,utcon[1],utcon[2],utcon[3],uconM[1],uconM[2],uconM[3],Rijtemp[0][1],Rijtemp[1][0]);
        
        
	int i,j;
	for(i=0;i<4;i++)
		for(j=0;j<4;j++)
			TijMink[i][j]=(rho + GAMMA*uint + bsq)*uconM[i]*uconM[j] - bconM[i]*bconM[j];  
        ...
        + cos(th)*cos(ph)*rho*utcon[2]*r
        - sin(ph)*rho*utcon[3]*r*sin(th);
        velvec[1] = sin(th)*sin(ph)*rho*utcon[1]
        + cos(th)*sin(ph)*rho*utcon[2]*r
        + cos(ph)*rho*utcon[3]*r*sin(th);
        velvec[2] = cos(th)*rho*utcon[1]
        - sin(th)*rho*utcon[2]*r;
        jvec[0]=(posvec[1]*velvec[2]-posvec[2]*velvec[1])*dx[1]*dx[2]*geomBL.gdet;
        jvec[1]=-(posvec[0]*velvec[2]-posvec[2]*velvec[0])*dx[1]*dx[2]*geomBL.gdet;
        jvec[2]=(posvec[0]*velvec[1]-posvec[1]*velvec[0])*dx[1]*dx[2]*geomBL.gdet;

        //Tilt and untilt?
        posvecT[0]=posvec[0];
        posvecT[1]=posvec[1]*cos(KT_DROT) - posvec[2]*sin(KT_DROT);
        posvecT[2]=posvec[1]*sin(KT_DROT) + posvec[2]*cos(KT_DROT);        
        
        //thp = fabs(acos(posvecT[2]/sqrt(posvecT[0]*posvecT[0] + posvecT[1]*posvecT[1] + posvecT[2]*posvecT[2])) -M_PI/2.);
        thp = acos(posvecT[2]/sqrt(posvecT[0]*posvecT[0] + posvecT[1]*posvecT[1] + posvecT[2]*posvecT[2]));
        
        //mean tilted coordinate based disk theta an rho?
        if(Be<0.) 
        {
          thm[ix][iz]+=thp*rho*dxph[1];
          mass+=rho*dxph[1];
        }
        
        velvecT[0]=velvec[0];
        velvecT[1]=velvec[1]*cos(KT_DROT) - velvec[2]*sin(KT_DROT);
        velvecT[2]=velvec[1]*sin(KT_DROT) + velvec[2]*cos(KT_DROT);
        
        jvecT[0]=posvecT[1]*velvecT[2]-posvecT[2]*velvecT[1];
        jvecT[1]=-(posvecT[0]*velvecT[2]-posvecT[2]*velvecT[0]);
        jvecT[2]=posvecT[0]*velvecT[1]-posvecT[1]*velvecT[0];        

        jvec[0]=(posvec[1]*TijMink[0][3] - posvec[2]*TijMink[0][2])*dx[1]*dx[2]*geomBL.gdet;
        jvec[1]=-(posvec[0]*TijMink[0][3] - posvec[2]*TijMink[0][1])*dx[1]*dx[2]*geomBL.gdet;
        jvec[2]=(posvec[0]*TijMink[0][2] - posvec[1]*TijMink[0][1])*dx[1]*dx[2]*geomBL.gdet;
        
        #ifdef RADIATION
        jvecR[0]=posvec[1]*FluxM[2] - posvec[2]*FluxM[1];
        jvecR[1]=-(posvec[0]*FluxM[2] - posvec[2]*FluxM[0]);
        jvecR[2]=posvec[0]*FluxM[1] - posvec[1]*FluxM[0];
        #endif
        
        if(isjet==0 && rho > rhomax*1e-4)
        {
          //if(ix==80) printf("%i %e %e %e %e %e \n",iz,r,utcon[3],jvec[0],jvec[1],jvec[2]);
          Jvec[0]+=jvec[0];//+jvecR[0]);
          Jvec[1]+=jvec[1];//+jvecR[1]);
          Jvec[2]+=jvec[2];//+jvecR[2]);
        }
        if(isjet==0)
        {
          //rbary or entropy? basic weighted quantity
          sgas_inside = pow(Ehat,3./4.);//uint * (GAMMA - 1.)/pow(rho,GAMMA);
          sgas = log(sgas_inside);
          //rbary+=r*rho*dx[0]*dx[1]*dx[2]*geomBL.gdet;
          rbary+=sgas*rho*dx[0]*dx[1]*dx[2]*geomBL.gdet;
          rbaryweight+=rho*dx[0]*dx[1]*dx[2]*geomBL.gdet;
        }
        else if(isjet==1)
        {
          jet_weight=0.5*(bsq/rho); //0.5*(bsq/rho)
          ...
          if(xxBL[2]<M_PI/2.)
          {
            xjet_top[0]+=jet_weight*xxveccar[1]*dx[1]*dx[2]*geomBL.gdet;
            xjet_top[1]+=jet_weight*xxveccar[2]*dx[1]*dx[2]*geomBL.gdet;
            xjet_top[2]+=jet_weight*xxveccar[3]*dx[1]*dx[2]*geomBL.gdet;
          }
          else
          {
            xjet_bot[0]+=jet_weight*xxveccar[1]*dx[1]*dx[2]*geomBL.gdet;
            xjet_bot[1]+=jet_weight*xxveccar[2]*dx[1]*dx[2]*geomBL.gdet;
            xjet_bot[2]+=jet_weight*xxveccar[3]*dx[1]*dx[2]*geomBL.gdet;
          }
                      
          jetpos_weight+=jet_weight*dx[1]*dx[2]*geomBL.gdet;
          
          printf("is jet =1 \n");
          printf("%e %e %e %e \n",jet_weight,xjet_top[0],xjet_top[1],xjet_top[2]);
        }
        
        //radiation flux direction
        if(xxBL[2]<M_PI/2. && isjet==1)
        {
          Rtilt_num+=cos(th)*(-Rrt)*dx[1]*dx[2]*geomBL.gdet;
          Rtilt_den+=(-Rrt)*dx[1]*dx[2]*geomBL.gdet;
          Rprec_num+=sin(th)*sin(ph)*(-Rrt)*dx[1]*dx[2]*geomBL.gdet;
          Rprec_den+=sin(th)*cos(ph)*(-Rrt)*dx[1]*dx[2]*geomBL.gdet;
        }
        
        Ssig[0]+= Tij[0][0]*geomBL.gdet*dx[0];
        Ssig[1]+= Tij[1][0]*geomBL.gdet*dx[0];
        Ssig[2]+= Tij[2][0]*geomBL.gdet*dx[0];
        Ssig[3]+= Tij[3][0]*geomBL.gdet*dx[0];
    ...
    //noramlize jet pos. by weights
    xjet_top[0] /= jetpos_weight;
    xjet_top[1] /= jetpos_weight;
    xjet_top[2] /= jetpos_weight;
    
    xjet_bot[0] /= jetpos_weight;
    xjet_bot[1] /= jetpos_weight;
    xjet_bot[2] /= jetpos_weight;
    
    //Standard disk angles
    ldouble tilt = acos( (Jvec[2])/(sqrt(Jvec[0]*Jvec[0]+Jvec[1]*Jvec[1]+Jvec[2]*Jvec[2])));
    ldouble prec = acos( (Jvec[1])/(sqrt(Jvec[0]*Jvec[0]+Jvec[1]*Jvec[1])));
    
    //Standard jet angles
    ldouble tilt_jet = acos( xjet_top[2]/sqrt(xjet_top[0]*xjet_top[0]+xjet_top[1]*xjet_top[1]+xjet_top[2]*xjet_top[2]));
    ldouble prec_jet = acos( xjet_top[1]/sqrt(xjet_top[0]*xjet_top[0]+xjet_top[1]*xjet_top[1]));
    
    //Trying stress-energy tensor method
    //ldouble tilt_jet = acos( (Jvec_jet[2])/(sqrt(Jvec_jet[0]*Jvec_jet[0]+Jvec_jet[1]*Jvec_jet[1]+Jvec_jet[2]*Jvec_jet[2])));
    //ldouble prec_jet = acos( (Jvec_jet[1])/(sqrt(Jvec_jet[0]*Jvec_jet[0]+Jvec_jet[1]*Jvec_jet[1])));
    
    //Radiation
    ldouble tilt_jet_rad = acos(Rtilt_num/Rtilt_den);
    ldouble prec_jet_rad = atan(Rprec_num/Rprec_den);
    
    //Check quadrant of J*JBH.x to see break degeneracy of arccos
    ldouble Jvec_dotx = -Jvec[0];
    ldouble Jvec_jet_dotx = -xjet_top[0];
    
    //if(Jvec_dotx < 0.) prec = 2.*M_PI - prec;
    //if(Jvec_jet_dotx < 0.) prec_jet = 2.*M_PI - prec_jet;
    profiles[53][ix]=tilt; //Disk tilt
    profiles[57][ix]=tilt_jet_rad; // Disk precession (59)
    profiles[58][ix]=prec_jet_rad; //Jet precession (60)
    profiles[59][ix]=tilt; //Disk tilt (61)
    profiles[60][ix]=tilt_jet; // Jet tilt (62)
    profiles[61][ix]=prec; // Disk precession (63)
    profiles[62][ix]=prec_jet; //Jet precession (64)
    
    //printf("ix Jx Jy Jz : %i %e %e %e\n",ix,Jvec[0],Jvec[1],Jvec[2]);
    ...
    //profiles[29][ix]=0.;
    //profiles[29][ix]=tan(profiles[29][ix]/2.);
    ...
    calc_lum_tausurface(xxBL[1],1.,&radlum);
    ...
    profiles[20][ix]=totallum;
    calc_lum(xxBL[1],4,&radlum,&totallum);
    //profiles[62][ix]=totallum;
    calc_lum(xxBL[1],5,&radlum,&totallum);
    profiles[63][ix]=totallum;
    //location of the photosphere (13)
    //profiles[11][ix]=calc_photloc(ix);
    profiles[11][ix]=rbary/rbaryweight;
    ...
    // special quantities for alpha Reynolds
    urdisk[ix]/=rhodisk[ix];
    uthdisk[ix]/=rhodisk[ix];
    uphidisk[ix]/=rhodisk[ix];
  ...
  for(ix=0;ix<NX;ix++)
  {
    ...
    for(iz=0;iz<NZ;iz++)
    {
      for(iy=0;iy<NY;iy++)
      {
        if(doingavg) 
        {
          pptemp=get_uavg(pavg,RHO,ix,iy,iz);
        }
        else
        {
          pptemp=get_u(p,RHO,ix,iy,iz);
        }
        //if(pptemp > rhomax) rhomax=pptemp;

        struct geometry geomBL;
        fill_geometry_arb(ix,iy,iz,&geomBL,OUTCOORDS);
        
        ldouble xx[4],xxBL[4],dx[3],dxph[3],r,th,ph,posvec[3],posvecT[3],xxveccar[4];
	...
	if(NZ==1) 
        ...
        ldouble thp = acos(posvecT[2]/sqrt(posvecT[0]*posvecT[0] + posvecT[1]*posvecT[1] + posvecT[2]*posvecT[2]));
        
        //mean tilted coordinate based disk theta
        thrho += pptemp*fabs(thp - thm[ix][iz])*dxph[1];
        mass+=pptemp*dxph[1];
      }
    }
    
    profiles[29][ix] = thrho/mass;
  }
...
int calc_scalars(ldouble *scalars,ldouble t)
{
 ...
 ldouble rmdot=rhorizonBL;
  ...
  //luminosities 
  ldouble rlum=500.;//rhorizonBL;
  ldouble rtot=5.;//rhorizonBL;
  ldouble rjw=500.;
  ldouble taulimit=1.;
  ...
  //calc_lum_tausurface(rlum,taulimit,&radlum);
  #if(PROBLEM==87)
  calc_lum_tausurface(rlum,taulimit,&radlum);
  #endif
  ...
  calc_lum(rtot,1,&radlum,&totallum);
  scalars[10]=totallum;//*mdotscale*CCC0*CCC0/calc_lumEdd();
  //printf("ltot = %e \n",scalars[10]);  

  //Storing  energy flux in 11  
  calc_lum(rjw,4,&radlum,&totallum);
  scalars[11]=totallum;//*mdotscale*CCC0*CCC0/calc_lumEdd();
  //printf("lj = %e \n",scalars[11]);
  
  //Storing  ang. mom. flux in 12  
  calc_lum(rjw,5,&radlum,&totallum);
  scalars[12]=totallum;//*mdotscale*CCC0*CCC0/calc_lumEdd();  
  //printf("lw = %e \n",scalars[12]);
  ...
  mdot=calc_mdot(rjw,3);
  scalars[3]=mdot*mdotscale/calc_mdotEdd();
...
calc_lum(ldouble radius,int type,ldouble *radlum, ldouble *totallum)
{
  ...
  else //non-sph symmetry
  {
        ...
        for(iy=0;iy<NY;iy++)
	{
	  ...
	  ldouble rhoucont,Trtmag,Ttt,Be,sigma,bcon[4],bcov[4],bsq; //Stuff for etacalc
          ldouble Rij[4][4],Rtt,ehat,ucongas[4],ucovgas[4];
          ldouble tautot[3],tau=0.;
          ldouble TrphiHD,Rrphi,TrtHD,Trphimag,Trphi;
          //ldouble gdet;
	  ...
	  if(doingavg)
	  {
		...
		- get_uavg(pavg,AVGBCONBCOV(1,0),ix,iy,iz); 
              ...
              TrtHD=get_uavg(pavg,AVGRHOUCONUCOV(1,0),ix,iy,iz)
		+ GAMMA*get_uavg(pavg,AVGUUUCONUCOV(1,0),ix,iy,iz);

              Trtmag=get_uavg(pavg,AVGBSQUCONUCOV(1,0),ix,iy,iz)
		- get_uavg(pavg,AVGBCONBCOV(1,0),ix,iy,iz);
	      ...
	      TrphiHD=get_uavg(pavg,AVGRHOUCONUCOV(1,3),ix,iy,iz)
		+ GAMMA*get_uavg(pavg,AVGUUUCONUCOV(1,3),ix,iy,iz);

	      Trphimag=get_uavg(pavg,AVGBSQUCONUCOV(1,3),ix,iy,iz)
		- get_uavg(pavg,AVGBCONBCOV(1,3),ix,iy,iz);  
             
              Trphi=Trphimag+TrphiHD;
             
	      Ttt=get_uavg(pavg,AVGRHOUCONUCOV(0,0),ix,iy,iz)
		+ GAMMA*get_uavg(pavg,AVGUUUCONUCOV(0,0),ix,iy,iz)
		+ get_uavg(pavg,AVGBSQUCONUCOV(0,0),ix,iy,iz)
		- get_uavg(pavg,AVGBCONBCOV(0,0),ix,iy,iz); ;
              ...
              #ifdef RADIATION
	      Rtt=get_uavg(pavg,AVGRIJ(0,0),ix,iy,iz);
	      Rrphi=get_uavg(pavg,AVGRIJ(1,3),ix,iy,iz);
	      ...
	      if(type==0) //R^r_t outside photosphere
		{
	      ...
	      else if(type==1  || type==4 || type==5) //positive R^r_t everywhere
		{
		  for(i=0;i<4;i++)
		    for(j=0;j<4;j++)
		      Rij[i][j]=get_uavg(pavg,AVGRIJ(i,j),ix,iy,iz);
		...
		Rrt=0.;
	      ...
	      if(Be >= 0.0 && uconr>0.) jetJ += -geomBL.gdet*(rhour+Trt)*dxBL[1]*dxBL[2];
	      //if(Be <= 0.05 && Be > 0. && uconr>0.) jetW += -geomBL.gdet*(rhour+Trt)*dxBL[1]*dxBL[2];

	      //jetJ += geomBL.gdet*(Trt+Rrt)*dxBL[1]*dxBL[2]; //energy flux
	      //jetW += -geomBL.gdet*(TrphiHD+Trphimag+Rrphi)*dxBL[1]*dxBL[2]; //angular momentum flux
	      
	      //jetJ += geomBL.gdet*(TrtHD+Rrt)*dxBL[1]*dxBL[2]; //energy flux
	      //jetW += -geomBL.gdet*(TrphiHD+Rrphi)*dxBL[1]*dxBL[2];; //angular momentum flux

	      //jetJ += geomBL.gdet*(TrtHD)*dxBL[1]*dxBL[2]; //energy flux
	      //jetW += -geomBL.gdet*(TrphiHD)*dxBL[1]*dxBL[2];; //angular momentum flux

	      //jetJ += geomBL.gdet*(Trtmag)*dxBL[1]*dxBL[2]; //energy flux
	      //jetW += -geomBL.gdet*(Trphimag)*dxBL[1]*dxBL[2];; //angular momentum flux	      
	      //printf("%i %i %i %e %e %e \n",ix,iy,iz,Be,jet,jetJ);
	  }
	  else //snapshot
	  { 
	      ...
	      tau+=ucongas[0]*tautot[1];
              ...
              //sigma calc
              #ifdef MAGNFIELD
              calc_bcon_bcov_bsq_from_4vel(pp, ucongas, ucovgas, &geomBL, bcon, bcov, &bsq);
              Trtmag=(bsq*ucongas[1]*ucovgas[0] - bcon[1]*bcov[0]);
              sigma = bsq/pp[RHO];
              #endif
	      ...
	      if(type==0) //R^r_t outside photosphere
		{
	      ...
	      else if(type==1 || type==4 || type==5) //R^r_t everywhere
		{
		  calc_Rij(pp,&geomBL,Rij); 
		  indices_2221(Rij,Rij,geomBL.gg);
		  Rrt=Rij[1][0];
...
#endif
	      jet+= -geomBL.gdet*(rhour+Trt+Rrt)*dxBL[1]*dxBL[2];
	      //if(Be > 0.05 && ucongas[1]>0.) jetJ += -geomBL.gdet*(rhour+Trt)*dxBL[1]*dxBL[2];
	      //if(Be <= 0.05 && Be > 0. && ucongas[1]>0.) jetW += -geomBL.gdet*(rhour+Trt)*dxBL[1]*dxBL[2];
	      jetJ += geomBL.gdet*(Trt+Rrt)*dxBL[1]*dxBL[2]; //energy flux
	      jetW += -geomBL.gdet*(Trt+Rrt)*dxBL[1]*dxBL[2];; //angular momentum flux
...
int
calc_lum_tausurface(ldouble radius, ldouble taumax,ldouble *radlum)
{
  ...
  if(NY>1) //non-sph symmetry
  {
    ...
    for(iz=0;iz<NZ;iz++)
    {
	  ...
	  ldouble xx[4],dx[3],pp[NV],Rrt,rhour,uintur,Tij[4][4],Trt,Trtmagn;
          ldouble Rij[4][4],Rtt,ehat,ucongas[4],ucovgas[4],bcon[4],bcov[4],bsq,sigma;
          //ldouble gdet;
...
#ifdef RADIATION
	      int i,j;
	      if(xxBL[1] <= radius || tau >= taumax)// || tau >=taumax) //R^r_t outside photosphere
		{
		  for(i=0;i<4;i++)
		    for(j=0;j<4;j++)
		      Rij[i][j]=get_uavg(pavg,AVGRIJ(i,j),ix,iy,iz);
		  ...
		  if(Rrt<0.) Rrt=0.;
                  ...
                  #endif
    	          lum+=geomBL.gdet*Rrt*dxBL[1]*dxBL[2];
    	          //lum+=geomBL.gdet*(-Trt -rhour + Rrt)*dxBL[1]*dxBL[2];
                  break;
		}
	      else
		Rrt=0.;
	  ...
	  { 
	      ...
	      Rrt=0.;
#ifdef RADIATION
	      if(xxBL[1] <= radius || tau >= taumax) //R^r_t outside photosphere
		{
		  calc_Rij(pp,&geomBL,Rij); 
		  indices_2221(Rij,Rij,geomBL.gg);
		  Rrt=-Rij[1][0];
		  if(Rrt<0.) Rrt=0.;
                  ...
                  //#endif
    	          lum+=geomBL.gdet*Rrt*dxBL[1]*dxBL[2];
    	          //printf("%i %e\n",iy,geomBL.gdet*Rrt*dxBL[1]*dxBL[2]);
                  break;
		}
	      else
		Rrt=0.;
...
calc_scaleheight(ldouble radius)
{
  ...
  ldouble scaleheight=0.,sigma=0.,rho,ugas;
...
#pragma omp parallel for private(iy,iz) reduction(+:sigma) reduction(+:scaleheight)
  for (iz = 0; iz < NZ; iz++)
  {
    for(iy = 5; iy < NY-5; iy++)
    {
      ...
      sigma += rho * dx[1];
      scaleheight += rho * dx[1] * pow(fabs(M_PI/2.-xxBL[2]),2.);
    }
  }
